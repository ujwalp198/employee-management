...
const AdminDashboard = ({ changeUser }) => {
  ...
  return (
    <div className='admin-layout-container admin-theme'>
      <AdminSidebar changeUser={changeUser} activeTab={activeTab} setActiveTab={setActiveTab} />
        ...
        <div className='admin-content-grid'>
          ...
          {activeTab === 'Analytics' && (
            <div className='admin-analytics-wrapper'>
              <h2 className='section-title'>Employee Performance Analytics</h2>
              <div className='admin-analytics-grid'>
                {userData && userData.map((employee) => {
                  ...
                  return (
                    <div key={employee.id} className='analytics-card glass-panel'>
                      <h3 className='analytics-name'>{employee.name} <span className='analytics-id'>({employee.id})</span></h3>
                      <div className='analytics-stats'>
                        <div className='stat-item'><span className='stat-label text-new'>New</span><span className='stat-val'>{newTaskCount}</span></div>
                        <div className='stat-item'><span className='stat-label text-active'>Active</span><span className='stat-val'>{activeCount}</span></div>
                        <div className='stat-item'><span className='stat-label text-completed'>Done</span><span className='stat-val'>{completedCount}</span></div>
                        <div className='stat-item'><span className='stat-label text-failed'>Failed</span><span className='stat-val'>{failedCount}</span></div>
                      </div>
                    </div>
                  );
                })}
          ...
          {activeTab === 'Employees' && (
            <div className='admin-employees-wrapper admin-card glass-panel'>
              <h2 className='section-title'>All Employees & Task Breakdown</h2>
                ...
                <table className='admin-table'>
                  <thead>
                    <tr>
                      ...
                      <th className='admin-th text-new'>New</th>
                      <th className='admin-th text-active'>Active/Accepted</th>
                      <th className='admin-th text-completed'>Done</th>
                      <th className='admin-th text-failed'>Failed</th>
                    </tr>
                  </thead>
                  <tbody>
                    {userData && userData.map((employee) => {
                      ...
                      const failedCount = employee.tasks.filter(t => t.failed).length;
                      return (
                        <tr key={employee.id} className='admin-tr'>
                          <td className='admin-td'><strong>{employee.id}</strong></td>
                          <td className='admin-td'>{employee.name}</td>
                          <td className='admin-td'>{newTaskCount}</td>
                          <td className='admin-td'>{activeCount}</td>
                          <td className='admin-td'>{completedCount}</td>
                          <td className='admin-td'>{failedCount}</td>
                        </tr>
                      );
                    })}
                  </tbody>
